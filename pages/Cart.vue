        ...
        <div class="cart__wrapper">
            <div class="cart__container">
                <div class="cart__header">
                    <h5 class="cart__title">"Корзина" в разработке, для оформления заказа позвоните по 311-04-24</h5>
                ...
                <Transition name="fade" mode="out-in">
                    ...
                    >
                        ...
                        >
                            ...
                            <div class="cart__sections" v-if="!list?.section?.dish">
                                <div
            ...
            <div class="cart__form-wrapper">
                ...
                >
                    <textarea
                            ...
                            v-model="message.comment" @change="statusMessage = ''" placeholder="Комментарий к заказу"
                            required
                            maxlength="256"
                            @input="resize($event)"
                    />
                    <input
                            ...
                            @input="validName"
                            v-model="message.name" @change="statusMessage = ''" placeholder="Имя" required
                            maxlength="32"
                    >
                    <input
                            ...
                            @input="validPhone"
                            v-model="message.phone" @change="statusMessage = ''" placeholder="Телефон" required
                            maxlength="20"
                    >
                ...
                <client-only>
                    <TransitionGroup name="slide-fade" mode="in-out" tag="div">
                        <div
                        ...
                        >
                            ...
                            >
                                ...
                                Ожидайте звонка от администратора ресторана для подтверждения заказа
                            </div>
                        </div>
                        <div
...
import menuList from '/src/api/json/menu-list.json';
import menuBar from '/src/api/json/mapBar-list.json';
import menuLunch from '/src/api/json/lunch-list.json';
export default {
    name: "cart",
    scrollToTop: true,
    data() {
    ...
    computed: {
        checked: {
            ...
            set(value) {
                this.store.setStatus(this.store, value)
            }
        }
    },
    ...
    methods: {
        ...
        loadingItemInCart() {
            ...
            Object.keys(cartStorage).map(async jsonName => {
                const api = this.API(jsonName)
                this.getDateForCart(api, cartStorage, jsonName)
            })
            this.cartKey += 1
        },
        API(jsonName) {
            if (jsonName === 'menu-list') {
                return menuList
            }
            if (jsonName === 'mapBar-list') {
                return menuBar
            }
            if (jsonName === 'lunch-list') {
                return menuLunch
            }
...
.cart {
  ...
  width: 100%;
  height: 100%;
  background: linear-gradient( rgba(256, 256, 256, 0.8), rgba(0, 0, 0, 0.5) ),url("/public/image/background/background-cart.jpg") 0 0 repeat;
  &__empty-basket {
    margin: 16px auto;
    font-size: 18px;
    line-height: 1.56;
